# NotificationSoundRouter — Project Context for Claude

## What This Is

A Windows 11 system tray app (C#, .NET 8) that intercepts toast notifications via WinRT `UserNotificationListener` and plays per-app custom `.wav` files. The global Windows notification sound is silenced via registry on startup and restored on exit.
...
- **Config:** `%AppData%\NotificationSoundRouter\config.json`
...
| `ConfigManager.cs` | Atomic JSON read/write, volatile config reference |
| `UI/BrandStyles.xaml` | Sidereal Cog brand ResourceDictionary |
| `UI/SettingsWindow.xaml/.cs` | Main settings UI |
| `UI/AppPickerDialog.xaml/.cs` | "Add app" picker |
| `Models/` | `RootConfig`, `AppConfig`, `SoundRule` — `AppConfig.Rules` is `ObservableCollection` |
| `Package.appxmanifest` | `rescap:userNotificationListener` capability; `ProcessorArchitecture="x64"` |
...
Self-managing apps (Discord, Slack desktop, Teams, Spotify) play sounds through their own audio engine. The `NotificationChanged` event still fires, so the app can play a sound — but the app's own sound may play simultaneously. Users should disable in-app sounds for these apps.
